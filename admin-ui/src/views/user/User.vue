...
<script setup>
  ...
  const getList = async (page = 1) => {
    const res = await http.post('/api/admin/user/list', { page, search: state.search })
  ...
  const editOk = async () => {
    ...
    if (state.editType === '添加') {
      res = await http.post('/api/admin/user/add', state.data)
    } else {
      res = await http.post('/api/admin/user/update', state.data)
    }
    await getList()
    if (res.data.success) {
  ...
  const remove = async data => {
    const res = await http.post('/api/admin/user/remove', { ids: data.id })
    ElMessage.success(res.data.message)
    await getList()
  }
