...
export class DeviceController {
  ...
  async list(@Body() body: any) {
    ...
    if (body.search.position) {
      knex = knex.where('position', 'like', `%${body.search.position}%`)
    }
  ...
  async add(@Body() body: any) {
    await this.knex.table('device').insert(body)
    return { success: true, message: '添加成功' }
  }

  @Post('update')
  async update(@Body() body: any) {
    await this.knex.table('device').where({ id: body.id }).update(body)
  ...
  async remove(@Body() body: any) {
    const ids = body.ids + ''
    await this.knex.table('device').where('id', 'in', ids.split(',')).delete()
